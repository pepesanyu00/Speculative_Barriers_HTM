...
#include "tmPower.h" //RIC meto los wrappers para iniciar y terminar transacción
...
ITYPE numThreads, exclusionZone, windowSize, tSeriesLength, profileLength, maxTileWidth, maxTileHeight, xactWidth;
...
#pragma omp for schedule(dynamic) nowait
      for (ITYPE tilej = tileii; tilej < profileLength; tilej += maxTileWidth)
      {
        ...
        //Paso los datos de mi profile privado al global y lo pongo a cero para el siguiente tile que procese
        for (ITYPE cnt = 0; cnt < maxTileWidth;)
        {
          BEGIN_TRANSACTION(tid, 0);
          for (ITYPE xcnt = 0; xcnt < xactWidth && cnt < maxTileWidth; xcnt++, cnt++)
          {
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    if (argc != 7)
    {
      cout << "usage: " << argv[0] << " input_file win_size tile_size xact_work num_threads dump_profile" << endl;
      ...
      cout << "       - xact_work: size of the transaction which dumps the private tile results to the global profile" << endl;
    ...
    xactWidth = atoi(argv[4]);
    numThreads = atoi(argv[5]);
    bool dumpProfile = (atoi(argv[6]) == 0) ? false : true;
    ...
    stringstream tmp;
    tmp << PATH_RESULTS << argv[0] << "_" << inputfilename.substr(inputfilename.rfind('/') + 1, inputfilename.size() - 4 - inputfilename.rfind('/') - 1) << "_w" << windowSize << "_l" << maxTileWidth << "_t" << numThreads << "_x" << xactWidth << "_d" << dumpProfile << "_" << getpid() << ".csv";
