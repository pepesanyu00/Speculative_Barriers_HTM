...
direc="../"
#"n100000-l100",
params=["n100000-l1000", "n100000-l10000",]
#"\\textbf{Barrier N=100K L=100}",
titles = ["\\textbf{Barrier N=100K L=1000}",
          "\\textbf{Barrier N=100K L=10000}",]
# [synchronization method, label, file]
files = [["PAR", "Parallel", "iwomp_%s-%s-t%d-*"],
         ["TM", "TM", "iwomp_%s-%s-t%d-*"],
         ["SB", "SB", "iwomp_%s-%s-t%d-*"],
         ["CS", "CS", "iwomp_%s-%s-t%d-*"],]
...
ini=150
fin=0
colors = plt.cm.Set1(range(ini,fin,(fin-ini)/(len(files)+3)))
colors[-3] = colors[-2]
mksize = (blw*6,blw*6,blw*5,blw*7,blw*5)
...
for i in range(len(params)):
  ...
  for j in range(len(files)):
    ...
    if (j%2) == 0:
      ax.plot(ind, seq/fbackPriv,color=colors[j+1],label=files[j][1], linewidth=blw,
  ...
  ax.set_yticklabels(np.arange(0,ymax+0.2,0.2), fontsize=lfs*0.8)
  ax.set_aspect(4.5/ymax)
  if i == 0: #Si es el último imprimo la leyenda
    lgd=ax.legend(frameon=True, fontsize=lfs, ncol=5, bbox_to_anchor=(2.15, 1.4), columnspacing=1)
...
plt.savefig("../z_figs/SUiwompP8.pdf", format='pdf',bbox_extra_artists=(lgd,),bbox_inches='tight')
plt.savefig("../z_figs/SUiwompP8.png", format='png',bbox_extra_artists=(lgd,),bbox_inches='tight')
