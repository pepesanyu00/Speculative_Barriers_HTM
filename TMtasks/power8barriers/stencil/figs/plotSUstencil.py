...
titles = ["\\textbf{Stencil T=100}",
          "\\textbf{Stencil T=200}",
          "\\textbf{Stencil T=400}",
          "\\textbf{Stencil T=800}",]
# [synchronization method, label, file]
files = [["UNP", "Unprotected", "stencil_%s-512-512-64-%s-%d-*"],
         ["PAR", "Parallel", "stencil_%s-512-512-64-%s-%d-*"],
         ["TM", "TM", "stencil_%s-512-512-64-%s-%d-*"],
         ["SB", "SB", "stencil_%s-512-512-64-%s-%d-*"],
         ["CS", "CS", "stencil_%s-512-512-64-%s-%d-*"]]
...
for i in range(len(params)):
  ...
  ymax = 35.0 #Lo pongo fijo para que todas la gráficas tengan el mismo ymax
...
plt.savefig("./SUstencilP8.pdf", format='pdf',bbox_extra_artists=(lgd,), bbox_inches='tight')
plt.savefig("./SUstencilP8.png", format='png',bbox_extra_artists=(lgd,), bbox_inches='tight')
