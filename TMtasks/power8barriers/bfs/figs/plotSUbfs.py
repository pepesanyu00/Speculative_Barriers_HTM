...
direc="../results/"
params=["1M_graph_input", "NY_graph_input"]
titles = ["\\textbf{BFS 1M graph}",
          "\\textbf{BFS NY graph}",]
# [synchronization method, label, file]
files = [["TM", "TM", "bfs_%s-%d-%s.dat-*"],
         ["SB", "SB", "bfs_%s-%d-%s.dat-*"],]
...
for i in range(len(params)):
  tmp = psf.StatsAvg(direc, (files[0][2]%("SEQ",1,params[i])), True)
  seq = np.array(tmp.GetTimeAvg())
  ax = plt.subplot(1, 2, i+1) #nrows, ncols, plot_number
  for j in range(len(files)):
    fbackPriv = []
    for d in numThreads:
      tmp = psf.StatsAvg(direc, (files[j][2]%(files[j][0],d,params[i])), False)
      tmp.ParseTimeAvg()
  ...
  if i == 0: #Si es el último imprimo la leyenda
    lgd=ax.legend(frameon=True, fontsize=lfs, ncol=2, bbox_to_anchor=(2.5, 1.42), columnspacing=1)
...
plt.savefig("./SUbfsP8.pdf", format='pdf',bbox_extra_artists=(lgd,), bbox_inches='tight')
plt.savefig("./SUbfsP8.png", format='png',bbox_extra_artists=(lgd,), bbox_inches='tight')
