...
void BFS_CPU(void *p) {
  ...
  while (!TMQUEUE_ISEMPTY(g_wavefront)) {
    ...
    /*assert(g_frontn > 0);
    chunk = g_frontn / nthreads;
    start = tid*chunk;
    stop = (tid == nthreads - 1) ? g_frontn : start + chunk;*/
    //printf("tid: %d frontn: %d chunk: %d start: %d stop: %d\n", tid, g_frontn, chunk, start, stop);
    TM_BARRIER(tid);
    if (tid == 0) g_index = (intptr_t) TMQUEUE_POP(g_wavefront);
    /*TM_BEGIN(tid, 0);
      if(!TMQUEUE_ISEMPTY(g_wavefront)) {
    ...
    chunk = (hgn[g_index].y + nthreads - 1) / nthreads;
    start = tid * chunk + hgn[g_index].x;
    stop = (start + chunk) > (hgn[g_index].y + hgn[g_index].x) ? (hgn[g_index].y + hgn[g_index].x) : start + chunk;
    //printf("tid: %d n: %d chunk: %d start: %d stop: %d\n", tid, hgn[g_index].y, chunk, start, stop);
