...
direc="../"
params=["n10000-c1", "n10000-c5", "n10000-c10", "n10000-c15"]
titles = ["\\textbf{Recurrence N=10000, C=1}",
          "\\textbf{Recurrence N=10000, C=5}",
          "\\textbf{Recurrence N=10000, C=10}",
          "\\textbf{Recurrence N=10000, C=15}",]
# [synchronization method, label, file]
files = [["UNP", "Unprotected", "recurrence_%s-%s-t%d-*"],
         ["PAR", "Parallel", "recurrence_%s-%s-t%d-*"],
         ["TM", "TM", "recurrence_%s-%s-t%d-*"],
         ["SB", "SB", "recurrence_%s-%s-t%d-*"],
         ["CS", "CS", "recurrence_%s-%s-t%d-*"]]
...
for i in range(len(params)):
  ...
  ymax = 25.0 #Lo pongo fijo para que todas la gráficas tengan el mismo ymax
...
#    ax.legend(loc="upper left", frameon=True, fontsize=lfs, labelspacing=0.2)
...
plt.savefig("../z_figs/SUrecurrenceP8-n10000.pdf", format='pdf',bbox_inches='tight')
plt.savefig("../z_figs/SUrecurrenceP8-n10000.png", format='png',bbox_inches='tight')
