...
int main(int argc, char *argv[])
{
  try
  {
    ...
    statsFile << setprecision(2) << fixed << (sizeof(DTYPE) * tSeries.size()) / 1024.0f << "," << (sizeof(DTYPE) * means.size()) / 1024.0f << "," << (sizeof(DTYPE) * norms.size()) / 1024.0f << "," << (sizeof(DTYPE) * df.size()) / 1024.0f << "," << (sizeof(DTYPE) * dg.size()) / 1024.0f << "," << (sizeof(DTYPE) * profileLength + sizeof(DTYPE) * maxTileWidth * 2 * numThreads) / 1024.0f << "," << (sizeof(ITYPE) * profileLength + sizeof(ITYPE) * maxTileWidth * 2 * numThreads) / 1024.0f << "," << ((sizeof(DTYPE) * tSeries.size()) / 1024.0f + (sizeof(DTYPE) * means.size()) / 1024.0f + (sizeof(DTYPE) * norms.size()) / 1024.0f + (sizeof(DTYPE) * df.size()) / 1024.0f + (sizeof(DTYPE) * dg.size()) / 1024.0f + (sizeof(DTYPE) * profileLength + sizeof(DTYPE) * maxTileWidth * 2 * numThreads) / 1024.0f + (sizeof(ITYPE) * profileLength + sizeof(ITYPE) * maxTileWidth * 2 * numThreads) / 1024.0f) / 1024.0f << endl;
