...
#elif defined(OMPCRI) /* openmp critical */
...
#pragma omp for schedule(dynamic) nowait
      for (ITYPE tilej = tileii; tilej < profileLength; tilej += maxTileWidth)
      {
        ...
        for (ITYPE jj = j; jj < MIN(tilej + maxTileWidth, profileLength); jj++)
        {
          ...
          {
            priv_profile_i[i - tilei] = correlation; //Actúo sobre el array global
        ...
        if (tilei != tilej)
        {
          ...
          for (ITYPE ii = i; ii < MIN(MIN(tilei + maxTileHeight, j - exclusionZone), profileLength); ii++)
          {
            ...
            {
              priv_profile_i[ii - tilei] = correlation; //Actúo sobre el array global
        ...
        for (ITYPE cnt = 0; cnt < maxTileWidth;)
        {
          ...
          for (ITYPE xcnt = 0; xcnt < xactWidth && cnt < maxTileWidth; xcnt++, cnt++)
          {
            ...
            priv_profile_i[cnt] = 0.0;
            priv_profile_j[cnt] = 0.0;
          }
          SYNC_END();
        }
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    if (argc != 6)
    {
      cout << "[ERROR] usage: ./scamp input_file win_size tile_size xact_work num_threads" << endl;
      return 1;
    }
    ...
    stringstream tmp;
    tmp << PATH_RESULTS << argv[0] << "_" << inputfilename.substr(inputfilename.rfind('/') + 1, inputfilename.size() - 4 - inputfilename.rfind('/') - 1) << "_w" << windowSize << "_l" << maxTileWidth << "_t" << numThreads << "_" << getpid() << ".csv";
