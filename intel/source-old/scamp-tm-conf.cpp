...
#pragma omp parallel
  {
    ...
    INIT_TRANSACTION(); //Declara las variables retries y status
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    if (argc != 5)
    {
      cout << "[ERROR] usage: ./scamp input_file win_size num_threads xact_length" << endl;
      return 1;
    }
    ...
    stringstream tmp;
    tmp << PATH_RESULTS << argv[0] << "_" << inputfilename.substr(inputfilename.rfind('/') + 1, inputfilename.size() - 4 - inputfilename.rfind('/') - 1) << "_w" << windowSize << "_t" << numThreads << "_x" << xactLength << "_" << getpid() << ".csv";
