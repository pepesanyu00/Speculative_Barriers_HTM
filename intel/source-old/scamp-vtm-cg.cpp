...
#pragma omp for schedule(dynamic)
    ...
    /* Cada iteración lleva VPACK elementos a la vez con las instrucciones vectoriales*/
    {
      ...
      */
      ...
      {
        if (correlation[ii] > profile[i])
        {
          profile[i] = correlation[ii];
          profileIndex[i] = j + ii;
        }
      }
      ...
      VDTYPE profilej_v = LOADU_PD(&profile[j]);
      VMTYPE mask = CMP_PD(correlation_v, profilej_v, _CMP_GT_OQ);
      MASKSTOREU_PD(&profile[j], mask, correlation_v);
      MASKSTOREU_EPI(&profileIndex[j], mask, SET1_EPI(i));
      COMMIT_TRANSACTION();
