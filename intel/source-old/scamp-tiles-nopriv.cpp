...
#pragma omp parallel
  {
    ...
    {
      //Con protección en el acceso al profile global no hace falta barrera
#if defined(RTM) || defined(HLE) || defined(OMPCRI)
#pragma omp for schedule(dynamic) nowait
#else //Sin protección en el acceso al profile hace falta barrera
#pragma omp for schedule(dynamic)
#endif
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    if (argc != 5)
    {
      cout << "[ERROR] usage: ./scamp input_file win_size tile_size num_threads" << endl;
      return 1;
    }
