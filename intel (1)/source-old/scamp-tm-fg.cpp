...
#if defined(RTM)
#define SYNC_BEGIN(tid,xid,lock) BEGIN_TRANSACTION(tid,xid)
...
#define SYNC_BEGIN(tid,xid,lock)  \
  {                               \
    unsigned long int __l = lock; \
    omp_set_lock(&(fg_locks[__l]));  \

#define SYNC_END()                  \
    omp_unset_lock(&(fg_locks[__l]));  \
  }
...
int main(int argc, char* argv[]) {
  try {
    ...
    //Uso 2 transacciones en mi código, paso 2 para que se creen
