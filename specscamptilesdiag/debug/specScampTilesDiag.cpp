...
//NO ES IMPORTANTE
        ...
        for (ITYPE jj = j; jj < MIN(tilej + maxTileWidth, profileLength); jj++)
        {
          ...
          //Triángulo superior
              ...
              #pragma omp critical
              {
                cout << " tileii:" << tileii << " tilej:" << tilej << " x:" << i << " y:" << jj << endl;
              }
          #endif
          covariance = 0;
          ...
          for (ITYPE jjj = jj + 1; jjj < MIN(tilej + maxTileWidth, profileLength); jjj++, i++)
          {
            ...
            correlation = covariance * norms[i] * norms[jjj];
              ...
              #pragma omp critical
              {
                cout << " tileii:" << tileii << " tilej:" << tilej << " x:" << i << " y:" << jjj << endl;
              }
            #endif
        ...
        if (tilei != tilej)
        {
          ...
          for (ITYPE ii = i; ii < MIN(MIN(tilei + maxTileHeight, j - exclusionZone), profileLength); ii++)
          {
            ...
            //Triángulo superior
              ...
              #pragma omp critical
              {
                cout << " tileii:" << tileii << " tilej:" << tilej << " x:" << ii << " y:" << j << endl;
              }
          #endif
            covariance = 0;
            for (ITYPE wi = 0; wi < windowSize; wi++)
                 ...
                 iii++, j++)
            {
                ...
                #pragma omp critical
                {
                  cout << " tileii:" << tileii << " tilej:" << tilej << " x:" << iii << " y:" << j << endl;
                }
              #endif
